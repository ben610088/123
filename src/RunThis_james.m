...
FlightPhase = 'TVC2_h10000V323alpha10_elevator5';
...
sysID_OBES.waveName                         = 'doublet';          % 激勵訊號的波形名稱，可選：'doublet'、'Orthogonal_Multisine'、...
test_case                                   = 'sysID_longitudinal';        % 可選：'no_sysID', 'sysID_all', 'sysID_longitudinal', 'sysID_lateral', ...
                                                                  % FEM test_case = 1 ~ 4  --> 全開
                                                                  % FEM test_case = 5 ~ 8  --> 縱向
                                                                  % FEM test_case = 9 ~ 12 --> 橫向
...
%         sim(mdl, [0 80])            % test
    ...
    otherwise
        tic
%         sim(mdl, [0 50.1])             % 指定特定時間，by 勁豪 & 家維 
        sim(mdl, [0 simTime])          % 執行sysID所需的時間，test
        toc
end
...
fileName = fullfile(proj.RootFolder,'\sysID\sysID_timeDomain\sysID_GeometryData_A1.mat');   % 飛機 Geometry data 的存檔位置
