...
% FlightPhase = 'TVC2_h10000V450alpha5';
run('PreLoadFcn.m')                                                % 載入初始參數設定

PIController.z.P    = 0.5;               % 建議值 0.5，家維 2022/04/09 修改成0.25
PIController.z.I    = 0.1;               % 建議值 0.1，家維 2022/04/09 修改成0.05



Mode_deflection_aileron  = 0;       % 偏轉模式： 0=傳統左右副翼差動；        1=左右副翼獨立偏轉
Mode_deflection_elevator = 0;       % 偏轉模式： 0=傳統左右升降舵同動；      1=左右升降舵獨立偏轉
Mode_deflection_rudder   = 0;       % 偏轉模式： 0=傳統左右方向舵差動；      1=左右方向舵獨立偏轉
Mode_deflection_TVC_z    = 0;       % 偏轉模式： 0=傳統左右向量噴嘴固定不動； 1=左右向量噴嘴左右獨立偏轉； 2=左右向量噴嘴左右同動
Mode_deflection_TVC_y    = 2;       % 偏轉模式： 0=傳統左右向量噴嘴固定不動； 1=左右向量噴嘴上下獨立偏轉； 2=左右向量噴嘴上下同動
...
test_case                                   = 'sysID_longitudinal';                  % 可選：'no_sysID', 'sysID_all', 'sysID_longitudinal', 'sysID_lateral', ...
    ...
    case 'doublet'
        ...
        switch test_case
            ...
            case 'sysID_lateral'
                ...
                sysID_OBES.TVC_rz.delay             = 10;    % 預設值  0秒 (與下個激勵訊號相差5秒)。子通道的延遲啟動時間
                sysID_OBES.TVC_ly.delay             = 0;    % 預設值 XX秒 (與下個激勵訊號相差5秒)。子通道的延遲啟動時間
                sysID_OBES.TVC_lz.delay             = 0;    % 預設值  5秒 (與下個激勵訊號相差5秒)。子通道的延遲啟動時間
                sysID_OBES.right_aileron.delay      = 15;   % 預設值 10秒 (與下個激勵訊號相差5秒)。子通道的延遲啟動時間
                sysID_OBES.left_aileron.delay       = 0;   % 預設值 15秒 (與下個激勵訊號相差5秒)。子通道的延遲啟動時間
                ...
                sysID_OBES.rudder.delay             = 5;   % 預設值 20秒 (與下個激勵訊號相差5秒)。子通道的延遲啟動時間
            otherwise
                disp('錯誤：  test_case 沒有這個選項，請檢查拼字是否正確')
        end
        ...
        sysID_OBES.right_aileron.amplitude  = 5;            % 預設值 8，子通道激勵訊號的振幅
        sysID_OBES.left_aileron.amplitude   = 5;            % 預設值 8，子通道激勵訊號的振幅
        sysID_OBES.right_elevator.amplitude = 1;            % 預設值 1，子通道激勵訊號的振幅
        sysID_OBES.left_elevator.amplitude  = 1;            % 預設值 1，子通道激勵訊號的振幅
        sysID_OBES.rudder.amplitude         = 2;            % 預設值 4，子通道激勵訊號的振幅

    case 'Orthogonal_Multisine'
        % Orthogonal_Multisine input
        run('create_Orthogonal_Multisine.m')
        Orthogonal_Multisine = inputSignal(:,2:end);
        switch test_case
            case 'no_sysID'

            case 'sysID_longitudinal'
                wavePeriod                          = T;                        % 預設值20秒，激勵訊號Orthogonal_Multisine的週期時間(秒)
            ...
            case 'sysID_lateral'
                ...
                sysID_OBES.TVC_rz.wave              = Orthogonal_Multisine(:,3);                         % 子通道激勵訊號的波形
                sysID_OBES.TVC_ly.wave              = 0;                         % 子通道激勵訊號的波形
                sysID_OBES.TVC_lz.wave              = Orthogonal_Multisine(:,3);                         % 子通道激勵訊號的波形
                ...
                sysID_OBES.rudder.wave              = Orthogonal_Multisine(:,2); % 子通道激勵訊號的波形
            otherwise
                disp('錯誤：  test_case 沒有這個選項，請檢查拼字是否正確')
                ...
                sysID_OBES.TVC_ry.amplitude         = 8;                         % 預設值 1，子通道激勵訊號的振幅
                sysID_OBES.TVC_rz.amplitude         = 3;                         % 預設值 1，子通道激勵訊號的振幅
                sysID_OBES.TVC_ly.amplitude         = 8;                         % 預設值 1，子通道激勵訊號的振幅
                sysID_OBES.TVC_lz.amplitude         = 3;                         % 預設值 1，子通道激勵訊號的振幅
                sysID_OBES.right_aileron.amplitude  = 1.8;                         % 預設值 1，子通道激勵訊號的振幅
                sysID_OBES.left_aileron.amplitude   = 1.8;                         % 預設值 1，子通道激勵訊號的振幅
                sysID_OBES.right_elevator.amplitude = 4;                         % 預設值 1，子通道激勵訊號的振幅
                sysID_OBES.left_elevator.amplitude  = 4;                         % 預設值 1，子通道激勵訊號的振幅
                sysID_OBES.rudder.amplitude         = 1.5;                         % 預設值 1，子通道激勵訊號的振幅
