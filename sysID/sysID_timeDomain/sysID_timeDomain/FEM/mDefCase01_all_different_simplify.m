...
%% Model definition
state_eq   = 'xdot_TC01_F16_all_different_simplify';% Function for state equations       
obser_eq   = 'obs_TC01_F16_all_different_simplify'; % Function for observation equations
Nx         = 12;                                    % Number of states 
Ny         = 18;                                    % Number of observation variables
Nu         = 17;                                    % Number of input (control) variables
NCl        = 11;                                    % Number of Cl prameters
NCm        = 11;                                    % Number of Cm prameters
...
% Z(:,1)  = data1(:,1);          % V    ，單位：ft/s
% Z(:,2)  = data1(:,2);          % alpha，單位：rad
% Z(:,3)  = data1(:,3);          % beta ，單位：rad
% Z(:,4)  = data1(:,4);          % phi  ，單位：rad
% Z(:,5)  = data1(:,5);          % theta，單位：rad
% Z(:,6)  = data1(:,6);          % psi  ，單位：rad
% Z(:,7)  = data1(:,7);          % p    ，單位：rad/s
% Z(:,8)  = data1(:,8);          % q    ，單位：rad/s
% Z(:,9)  = data1(:,9);          % r    ，單位：rad/s
% Z(:,10) = data1(:,10);         % pdot ，單位：rad/s^2
% Z(:,11) = data1(:,11);         % qdot ，單位：rad/s^2
% Z(:,12) = data1(:,12);         % rdot ，單位：rad/s^2
% Z(:,13) = data1(:,13);         % ax   ，單位：ft/s^2
% Z(:,14) = data1(:,14);         % ay   ，單位：ft/s^2
% Z(:,15) = data1(:,15);         % az   ，單位：ft/s^2
% Z(:,16) = data1(:,16);         % mu   ，單位：rad
% Z(:,17) = data1(:,17);         % chi  ，單位：rad
% Z(:,18) = data1(:,18);         % gamma，單位：rad
...
% Cm, dC_m, Cm_delta_ra, Cm_delta_la, dCm_ds, Cm_elevator0, Cm_lef, Cm_q, dCm_q_lef, dCm_sb, eta_elevator
